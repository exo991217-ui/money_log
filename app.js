...
const DEFAULT_DATA=()=>({
  ...
  budgetCategories:[
    {id:101,name:'식비',budget:200000},
    {id:102,name:'교통',budget:50000},
    {id:103,name:'쇼핑',budget:100000},
    {id:104,name:'문화/여가',budget:50000},
    {id:105,name:'기타',budget:30000},
  ],
  monthBudgets:{},
  budgetSync:true,
  ledgerCategories:[
...
function loadState(){
  try{
    ...
    if(raw){
      ...
      if(S.budgetSync===undefined)S.budgetSync=true;
...
// Budget cats: global when sync ON, per-month when sync OFF
function getActiveBudgetCats(y,m){
  if(S.budgetSync)return S.budgetCategories||[];
  const key=mkey(y,m);
  return (S.monthBudgets&&S.monthBudgets[key])?S.monthBudgets[key]:(S.budgetCategories||[]);
}

function renderAll(){
  renderDashboard();renderIncome();renderCredit();renderAssets();
  renderStocks();renderCalendar();renderFood();renderInstallment();
...
function renderBudget(y,m){
  ...
  // Sync checkbox state
  const syncCheck=document.getElementById('budget-sync-check');
  if(syncCheck)syncCheck.checked=!!S.budgetSync;
  const syncStatus=document.getElementById('budget-sync-status');
  if(syncStatus){
    if(S.budgetSync){
      syncStatus.textContent='✅ 예산 동기화 중 (모든 달 공유)';syncStatus.style.display='inline';
    } else {
      syncStatus.textContent='';syncStatus.style.display='none';
    }
  }
    ...
    return `
      <div class="budget-cat-row">
        <div class="budget-cat-top">
          <span class="budget-cat-name">${cat.name}${foodBadge}</span>
...
function saveBudgetCategory(){
  ...
  if(S.budgetSync){
    ...
    if(id){const c=S.budgetCategories.find(c=>c.id==id);if(c){c.name=name;c.budget=budget;}}
    else S.budgetCategories.push({id:genId(),name,budget});
  } else {
    const key=mkey(cm.y,cm.m);
    if(!S.monthBudgets)S.monthBudgets={};
    if(!S.monthBudgets[key])S.monthBudgets[key]=[...(S.budgetCategories||[]).map(c=>({...c}))];
    const arr=S.monthBudgets[key];
    if(id){const c=arr.find(c=>c.id==id);if(c){c.name=name;c.budget=budget;}}
    else arr.push({id:genId(),name,budget});
  }
...
function deleteBudgetCategory(id){
  ...
  if(S.budgetSync){
    ...
    if(!S.monthBudgets)S.monthBudgets={};
    if(!S.monthBudgets[key])S.monthBudgets[key]=[...(S.budgetCategories||[]).map(c=>({...c}))];
    S.monthBudgets[key]=S.monthBudgets[key].filter(c=>c.id!=id);
  }
...
function toggleBudgetSync(checked){
  ...
  const key=mkey(cm.y,cm.m);
  if(checked){
    // Merge current month's budget back to global if it exists
    if(S.monthBudgets&&S.monthBudgets[key])S.budgetCategories=S.monthBudgets[key];
    S.budgetSync=true;
  } else {
    // Fork: copy global to this month
    if(!S.monthBudgets)S.monthBudgets={};
    S.monthBudgets[key]=[...(S.budgetCategories||[]).map(c=>({...c,id:c.id||genId()}))];
    S.budgetSync=false;
  }
  saveState();renderBudget(cm.y,cm.m);
}
...
function renderDashboard(){
  ...
  document.getElementById('dash-banner-sub').textContent=bannerIsLedger
    ?'📒 가계부 수입 '+fmt(ledBannerIn)+' − 지출 '+fmt(ledBannerOut)
  ...
  document.getElementById('dash-food-total').textContent=fmt(foodTotal);
  document.getElementById('dash-credit-total').textContent=fmt(totalCredit);
  ...
  renderDashExpand('food',[{name:'식비 캘린더',cat:'이번 달 합계',amount:foodTotal,color:'green'}]);

  const creditItems=[];
  for(const card of S.creditCards){
    if(isCardDueInMonth(card,cm.y,cm.m)){
      const monthly=Math.ceil(card.amount/card.months);
      const paid=(card.paidMonths||[]).includes(mkey(cm.y,cm.m));
      creditItems.push({name:card.card+'—'+card.item,cat:paid?'결제완료':'미결제',amount:monthly,color:paid?'green':'blue'});
    }
  }
  renderDashExpand('credit',creditItems);
...
async function fetchStockPrices(){
  ...
  saveState();renderStocks();renderDashboard();
...
function renderCalendar(){
  ...
  grid.innerHTML=months.map((mLabel,idx)=>{
    ...
    const monthTotal=events.reduce((s,e)=>s+(parseFloat(e.amount)||0),0);
    return `
...
function saveAsset(){
  ...
  if(id){const a=S.assets.find(a=>a.id==id);if(a){a.name=name;a.amount=amount;}}
  else S.assets.push({id:genId(),name,amount});
...
function saveStock(){
  ...
  saveState();closeModal();renderStocks();renderDashboard();
}
...
function deleteItem(type,id){
  ...
  else if(type==='stock'){S.stocks=S.stocks.filter(s=>s.id!=id);}
  saveState();renderAll();
}
...
function updateStockPrice(id,val){const st=S.stocks.find(s=>s.id==id);if(st)st.currentPrice=parseFloat(val)||0;saveState();renderStocks();renderDashboard();}
...
async function downloadMonthlyReport(){
  ...
  try{
    ...
    // Tips
    const varPct=totalExpense>0?(totalVar+foodTotal)/totalExpense*100:0;
    const tip=varPct<30?'고정지출 중심으로 안정적인 소비 패턴이에요. 여유 자금을 조금 더 활용해보세요!':varPct>60?'변동지출 비중이 높아요. 카테고리별로 지출을 분석해 보세요!':'균형 잡힌 소비 패턴이에요. 꾸준히 유지해 보세요!';
    const savTip=savingsRate>=30?'적금과 투자로 미래를 위한 준비를 착실히 하고 있어요!':savingsRate>=15?'저축을 잘 하고 있어요. 목표를 높여볼까요?':'저축률을 조금 더 높이면 좋을 것 같아요!';
    // Goals
    const goals=S.savingsGoals[y]||[];
...
<div style="width:${W}px;font-family:'Apple SD Gothic Neo','Noto Sans KR','Malgun Gothic',sans-serif;background:#fff;color:#2D2D3A;line-height:1.4;">
  <!-- HEADER -->
  <div style="background:linear-gradient(135deg,#5E4BC4 0%,#A29BFE 60%,#74B9FF 100%);padding:28px 36px;color:white;display:flex;justify-content:space-between;align-items:flex-start;">
    <div>
  ...
  <div style="display:flex;gap:16px;padding:0 24px 16px;background:#F7F4FF;">
    <div style="flex:1;background:white;border-radius:16px;padding:18px 20px;border:1.5px solid #EEE9FF;box-shadow:0 2px 12px rgba(162,155,254,.10);">
      <div style="font-size:13px;font-weight:800;margin-bottom:12px;">이번 달 기록</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
        ${[
          {icon:'📒',label:'총 거래 건수',val:entries.length+'건'},
          {icon:'✅',label:'무지출 일수',val:zeroDays+'일'},
          {icon:'📅',label:'1일 평균 지출',val:fmt(avgPerDay)},
        ].map(r=>`<div style="background:#F7F4FF;border-radius:10px;padding:10px 12px;text-align:center;">
          <div style="font-size:18px;margin-bottom:4px;">${r.icon}</div>
          <div style="font-size:10px;color:#9490A8;">${r.label}</div>
          <div style="font-size:14px;font-weight:800;margin-top:2px;">${r.val}</div>
        </div>`).join('')}
      </div>
    </div>
    <div style="flex:1.5;background:white;border-radius:16px;padding:18px 20px;border:1.5px solid #EEE9FF;box-shadow:0 2px 12px rgba(162,155,254,.10);">
      <div style="font-size:13px;font-weight:800;margin-bottom:12px;">이번 달 한 줄 요약</div>
      <div style="background:#F5F0FF;border-radius:12px;padding:14px 16px;border-left:4px solid #A29BFE;">
        <div style="font-size:12px;color:#5E4BC4;line-height:1.6;">${tip}</div>
        <div style="font-size:12px;color:#43C98A;margin-top:6px;line-height:1.6;">${savTip}</div>
      </div>
    </div>
  </div>
  <!-- 소비팁 + 다음달 목표 -->
  <div style="display:flex;gap:16px;padding:0 24px 20px;background:#F7F4FF;">
    <div style="flex:1;background:white;border-radius:16px;padding:18px 20px;border:1.5px solid #EEE9FF;box-shadow:0 2px 12px rgba(162,155,254,.10);">
      <div style="font-size:13px;font-weight:800;margin-bottom:10px;">💡 소비 팁</div>
      <div style="font-size:12px;color:#5E4BC4;line-height:1.7;background:#F5F0FF;padding:10px 14px;border-radius:10px;">
        이번 달 고정지출 비중이 ${totalExpense>0?Math.round(totalFixed/totalExpense*100):0}%예요.<br/>변동지출을 조금 더 활용하면 여유 자금이 늘어날 수 있어요!
      </div>
    </div>
    <div style="flex:1;background:white;border-radius:16px;padding:18px 20px;border:1.5px solid #EEE9FF;box-shadow:0 2px 12px rgba(162,155,254,.10);">
      <div style="font-size:13px;font-weight:800;margin-bottom:10px;">🎯 다음 달 목표</div>
      ${goalsList.map(g=>`<div style="display:flex;align-items:center;gap:8px;margin-bottom:7px;font-size:12px;"><span style="color:#43C98A;font-size:14px;">✓</span><span>${g}</span></div>`).join('')}
    </div>
  </div>
  <!-- FOOTER -->
  <div style="background:#5E4BC4;padding:14px 36px;display:flex;justify-content:space-between;align-items:center;color:white;">
...
// ===== APP EXPORT =====
window.App={
  changeMonth,changeCalYear,toggleDashSection,
  ...
  editItem,deleteItem,
  updateAssetAmount,updateStockPrice,
